...
from omnimemory.core.config import DEFAULT_N_RESULTS, DEFAULT_MAX_MESSAGES
from omnimemory.core.utils import format_conversation, _MemoryBatcher
...
class OmniMemorySDK:
    ...
    Provides high-level methods for memory operations including creation, querying,
    deletion, and evolution chain traversal. Supports both synchronous and
    asynchronous operations with background task management.
    """

    def __init__(self) -> None:
        """
        Initialize OmniMemorySDK with LLM connection .

        Sets up the LLM connection and initializes internal data structures
        for managing background tasks and memory batchers.
        """
        self.llm_connection: LLMConnection = LLMConnection()
        self._memory_manager: Optional["MemoryManager"] = None
        self._background_tasks: Dict[str, asyncio.Task] = {}
        self._batchers: Dict[Tuple[str, str, str], _MemoryBatcher] = {}
    ...
    def memory_manager(self) -> "MemoryManager":
        ...
        Returns:
            MemoryManager instance with connection pooling support.
        """
    ...
    def _register_background_task(self, task_id: str, task: asyncio.Task) -> None:
        """
        Track lifecycle of background asyncio tasks.

        Registers a task and sets up automatic cleanup when the task completes.
    ...
    async def add_memory(self, user_message: UserMessages) -> dict:
        ...
        """
        try:
            formatted_message = format_conversation(user_message)
    ...
    async def summarize_conversation(
        ...
        Args:
            summary_request: ConversationSummaryRequest containing app_id, user_id,
                           session_id, messages, and optional callback configuration.

        Returns:
            Dictionary with summary data (sync) or task metadata (async callback mode).

        Raises:
            Exception: If summary generation fails in sync mode.
        """
        ...
        async def _run_summary(use_fast: bool = False) -> Dict[str, Any]:
            return await self.memory_manager.generate_conversation_summary(
                ...
                messages=format_conversation(summary_request.messages),
                llm_connection=self.llm_connection,
    ...
    async def add_agent_memory(self, agent_request: AgentMemoryRequest) -> dict:
        """
        Add memory from agent message and trigger asynchronous memory processing .
        ...
        try:
            ...
            async def _process_agent_memory_background():
                ...
                try:
                    # Format messages using utils (single source of truth)
                    formatted_messages = format_conversation(agent_request.messages)
    ...
    def _batcher_key(
        self, app_id: str, user_id: str, session_id: Optional[str]
    ) -> Tuple[str, str, str]:
        """
        Generate a unique key for a memory batcher instance.
        ...
        Returns:
            Tuple of (app_id, user_id, session_id_or_empty_string) used as a key.
        """
        return (app_id, user_id, session_id or "")

    def _get_memory_batcher(
        self, app_id: str, user_id: str, session_id: Optional[str]
    ) -> "_MemoryBatcher":
        """
        Get or create a memory batcher instance for the given identifiers.
        ...
        Returns:
            MemoryBatcher instance for the given identifiers.
        """
        key = self._batcher_key(app_id, user_id, session_id)
        if key not in self._batchers:
            self._batchers[key] = _MemoryBatcher(
                sdk=self,
                app_id=app_id,
                user_id=user_id,
                session_id=session_id,
    ...
    def _maybe_cleanup_batcher(
        self, app_id: str, user_id: str, session_id: Optional[str]
    ) -> None:
        """
        Remove a memory batcher if it can be cleaned up.
        ...
        if batcher and batcher.can_cleanup:
            self._batchers.pop(key, None)

    async def memory_batcher_add(
        self, request: MemoryBatcherAppendRequest
    ) -> Dict[str, Any]:
        """
        Append messages to the in-memory MemoryBatcher buffer.

        Args:
            request: MemoryBatcherAppendRequest containing app_id, user_id, session_id, and messages.

        Returns:
            Dictionary with batcher status and result information.
        """
        batcher = self._get_memory_batcher(
            request.app_id, request.user_id, request.session_id
        )
        result = await batcher.add_messages(request.messages)
        self._maybe_cleanup_batcher(request.app_id, request.user_id, request.session_id)
        return result

    async def memory_batcher_add_message(
        self,
        app_id: str,
        user_id: str,
        role: str,
        ...
        """
        Append a single role/content message to the MemoryBatcher.
        ...
        request = MemoryBatcherAppendRequest(
            ...
            user_id=user_id,
            session_id=session_id,
            messages=[MemoryBatcherMessage(role=role, content=content)],
        )
        return await self.memory_batcher_add(request)
    ...
    async def _post_callback(
        ...
        Args:
            url: Callback URL to send the payload to.
            payload: Dictionary containing the summary data to send.
            headers: Optional HTTP headers to include in the request.

        Raises:
            httpx.HTTPStatusError: If the request fails after all retries.
            Exception: If the request fails with a non-HTTP error after all retries.
        """
        max_retries = 3
        base_delay = 1.0
    ...
    async def get_connection_pool_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing connection pool statistics, or error information if failed.
        """
        try:
    ...
    #     """
    #     Get the status and result of a memory creation task .
